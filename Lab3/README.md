...
### Task
- Use 5 __http__ methods to send requests to _https://httpbin.org/_:
...
- The __GET__ method is used to retrieve some data from a server, in my case, I'm retrieving an svg image with this response:
...
- The __POST__ method is used to send data to a server. I send a couple of parameters, since we don't have forms. Here's the response:
...
```json
{
  ...
  "json": null,
  "origin": "93.116.186.149, 93.116.186.149",
  "url": "https://httpbin.org/post"
}
```

- The __DELETE__ method is used to remove a target resource from the server, the response is:
...
- The __PUT__ method replaces the target resource with a specified content, in my case I sent some parameters again:
...
- The __PATCH__ method is like __PUT__ except it replaces only partially the resource (no parameters sent):
